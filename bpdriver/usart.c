...
void usart_sendstring(USART_TypeDef * usartn, char * format){
	while(*format){
		...
		while(USART_GetITStatus(USART1,USART_IT_TXE) != RESET);	//发送寄存器为empty
		format++;
	}
}
...
//	EXTI_InitTypeDef EXTI_InitStructure;
	...
	USART_ITConfig(USART1, USART_IT_RXNE, ENABLE);	//中断寄存器满时产生中断
}	
...
//	EXTI_InitTypeDef EXTI_InitStructure;
	...
	USART_ITConfig(USART2, USART_IT_RXNE, ENABLE);	//中断寄存器满时产生中断
}	
